# Insight Links — Creative Do's & Don'ts Guide (April 2026)

*For copywriters, creative strategists, and designers working on Cardiac Wire, Imaging Wire, and Digital Health Wire ad campaigns.*

*Last updated: March 22, 2026 | Sources: Client intelligence file, deep enrichment report, client-config, meeting transcripts, Slack history, Reddit audience research.*

---

## Cardiac Wire (CW) — Cardiologists & Cardiac Specialists

### Audience ICP
- **Primary:** Practicing cardiologists, interventional cardiologists, cardiac surgeons, electrophysiologists
- **Secondary:** Cardiac department heads, cardiology group practice CEOs, cardiology fellows/residents (Jake confirmed residents are "Good" — "always happy to have next generation of cardiologists as readers")
- **Landing page:** cardiacwire.com/subscribe-feed
- **Social proof on LP:** 39,000+ cardiology leaders
- **LP headline:** "Healthcare can be complicated. Your cardiology news shouldn't be."
- **Current performance:** $1.44 raw CPL | $15.57 qualified CPL | 47.39% CVR

### DO's
- **Lead with the "overwhelmed by reading" pain point.** DCT_118 (Text Callout) is the #1 CW performer ($1.29 CPL, 4.09% CTR, +53% qualified leads WoW) because it directly addresses "feeling you need to read/be on top of everything all the time." This is the winning CW angle.
- **Use text-heavy static formats.** DCT_118 is a text callout static — not a video, not a polished graphic. CW cardiologists respond to direct, clean copy over visual complexity.
- **Write peer-to-peer.** The confession format works ("Confession: If you're a cardiologist who wants to improve patient outcomes, you need to read this"). It feels like a colleague sharing a tip, not a brand selling a product.
- **Signal professional filtering explicitly.** DCT_124 (Cardiologist Filter) is in the pipeline specifically to improve CW qualification rates. Hooks that make clear this is FOR cardiologists and NOT for general audiences help pre-qualify. Example direction: "This newsletter exists for one audience: cardiologists who need to stay sharp without drowning in journals."
- **Use time-saving language.** Approved phrases: "stay current," "caught up fast," "filter the noise," "smarter while saving time." CW readers are time-starved physicians — every hook should promise time back.
- **Reference the volume of cardiology news as the problem.** The "maze of information" and "I can never catch up" voice-of-customer phrases resonate. The enemy is not ignorance — it is information overload from too many sources.
- **Age-target 25-59.** This was a hard-won fix. Early CW campaigns pulled majority 65+ leads (retired, non-practicing), which tanked qualification rates from 4.2% to the current 47.39% after the age targeting correction.

### DON'Ts
- **Do NOT use vague "information overload" angles without specificity.** DCT_120 (Information Overload) was killed on CW: $5.52 CPL, 0.84% CTR after 3 weeks. The lesson: "information overload" as a concept is too generic. You must ground it in cardiology-specific context (e.g., "keeping up with ACC guidelines, ESC updates, and clinical trial results" — not just "too much info").
- **Do NOT use language that sounds like a sales pitch.** DCT_109 ("The Best Radiologists Newsletter") was killed at $12.15 CPL. Superlative claims ("the best," "the #1") feel like marketing to clinicians, not peer recommendation.
- **Do NOT write copy that could apply to any newsletter.** CW copy must feel cardiology-specific. Generic "stay informed" language without cardiology anchors underperforms.
- **Do NOT rely solely on DCT_103 (Confession) for CW volume anymore.** Jake flagged on March 19 that DCT_103 is producing "nearly all non-target readers" — creative fatigue is real. Budget is shifting to DCT_118. New CW concepts should build on the Text Callout and Cardiologist Filter angles, not iterate further on Confession.
- **Do NOT speak to or about patients.** CW creative that could be interpreted as patient-facing will attract non-qualified leads (patients Googling cardiology information). Frame everything around the practitioner's professional workflow.
- **Do NOT open targeting to 60+.** The 65+ demographic was the single biggest quality problem in CW's history. Any creative or targeting change that re-opens this will trigger immediate client concern.

---

## Imaging Wire (IW) — Radiologists & Imaging Professionals

### Audience ICP
- **Primary:** Radiologists (diagnostic and interventional), radiology department heads, imaging center medical directors
- **Secondary:** Radiology fellows, imaging physicists, radiology administrators at named practices/hospital systems
- **Landing page:** theimagingwire.com/subscribe-feed
- **Social proof on LP:** 32,000+ imaging leaders
- **LP headline:** "Healthcare can be complicated. Your radiology news shouldn't be."
- **Current performance:** $3.96 raw CPL | $22.36 qualified CPL | 34.59% CVR

### Critical Context
Imaging Wire generates approximately 65% of Insight Links' total revenue. Jake's quote: *"Imaging Wire is the problem. And technically that's like where we still make, shoot, I don't know, 65% of our revenue. So like we need it to work well."* IW underperformance is what triggered the Growletter bake-off. IW is currently the only newsletter above the $20 qualified CPL target. Every IW creative decision carries outsized weight.

### DO's
- **Use the "Replace Journals" transformation angle.** DCT_114 (ReplaceJournals) is IW's quality champion: $2.39 CPL, 3.60% CTR, and a 75% qualification rate. The before/after framing — replacing dense RSNA/AJR journals with an accessible 5-minute read — directly addresses a validated radiology pain point.
- **Show visual proof of value.** The Apple Notes / transformation format (screenshot of notes app with newsletter content) works because it demonstrates rather than claims. If creating IW static creative, show what "catching up on radiology news" actually looks like in practice.
- **Position against legacy information sources.** IW competitors are AuntMinnie, Radiology Business, Applied Radiology, RSNA Radiology. The differentiator: those are either dense academic journals or community forums. IW is "human voice" curation. Creative should lean into: "written so you actually read it" vs. "another RSNA abstract you bookmark and forget."
- **Explore the "rounds" metaphor.** DCT_126 (WeeklyRounds) is in the pipeline — a confession-style angle using the familiar "rounds" concept from clinical practice. Radiologists do morning readout sessions; framing the newsletter as "your professional rounds, delivered" could resonate.
- **Use the "Stay Current Without The Burden" framing.** DCT_119 debuted at $1.99 CPL on IW and is being monitored as the DCT_120 replacement. "Without the burden" is a stronger qualifier than "without the work" — it acknowledges that staying current feels like an obligation, not a task.
- **Signal credibility and domain expertise in every ad.** Reddit audience research surfaced this: *"Summaries worry me unless I know they're done by people who understand the field."* IW creative must signal that the curation is done by people who understand radiology — not a generic AI summary or news aggregator.
- **Test the "Private Briefing" angle.** DCT_123_PrivateBriefing is in the pipeline for IW: peer pressure + social proof, text-heavy static. The premise: "The briefing 32,000 imaging leaders already read" — social proof works when the number is credible and the audience is niche enough to feel exclusive.

### DON'Ts
- **Do NOT let IW creative stagnate.** This is the single most dangerous thing you can do on this account. Jake's exact words about Growletter: *"for like six months... they would say, Imaging Wire is still above target, we're going to try some new things and then the next week just on and on."* That pattern of reporting problems without solving them is what killed the relationship. If IW creatives plateau, new concepts must ship immediately.
- **Do NOT run generic "Why read X?" angles on IW.** This format hits $7.36 CPL on IW (vs. $4.14 on DHW). The IW audience is more niche and skeptical than DHW. They need specificity, not broad value propositions.
- **Do NOT use "The Best Radiology Newsletter" or similar superlatives.** DCT_109 was killed. Radiologists are evidence-based professionals — a superlative claim with no proof reads as spam.
- **Do NOT create IW ads that could attract nurses.** In the Growletter bake-off, GL's IW pool was "almost entirely nurses — zero radiologists." TFM delivered 12 actual radiologists at named practices. The quality bar for IW is "actual radiologists at real practices," not "healthcare workers interested in imaging."
- **Do NOT ignore IW's 34.59% CVR problem.** IW's landing page converts below the 40% benchmark. Until the LP is optimized, creative must do more pre-selling work. Longer primary text that sets expectations and pre-qualifies before the click can compensate for a weaker LP.
- **Do NOT deprioritize IW creative cycles.** IW is at $22.36 qualified CPL — above the $20 target. It needs the most creative attention of all three newsletters, not the least.

---

## Digital Health Wire (DHW) — Digital Health Executives & Health Tech Leaders

### Audience ICP
- **Primary (Clinician track):** Digital health executives, CTOs/CIOs at health systems, health tech founders, health IT directors
- **Primary (Non-clinician track):** Healthcare administrators, EHR implementation leads, clinical software buyers, health IT procurement leaders — Jake's words: *"Our DHW advertisers want the guys who BUY this software, who aren't nurses/doctors (they are the users but not the buyers)."*
- **Landing page:** digitalhealthwire.com/subscribe-feed
- **Social proof on LP:** 28,000+ digital health leaders
- **LP headline:** "Healthcare can be complicated. Your news shouldn't be."
- **Current performance:** $2.77 raw CPL | $13.44 qualified CPL | 47.65% CVR

### DO's
- **Use the "If you, BUT" conditional hook for non-clinician targeting.** DCT_123 is DHW's best-ever performer: $1.51 CPL, 3.26% CTR. The conditional framing pre-qualifies the reader. Jake-approved example: *"If you work in healthcare BUT you're not a clinician and instead your job involves EHR systems, clinical software, or health IT, this newsletter is for you."*
- **Distinguish between clinician and non-clinician creative tracks.** DHW serves two audiences. Clinician-track creative speaks to digital health executives and health tech founders. Non-clinician-track creative speaks to "the guys who BUY the software" — healthcare admin, IT leaders, procurement. Use different hooks for each track.
- **Use specific health IT terminology for non-clinician ads.** Jake approved: "EHR systems, clinical software, or health IT" as specific language for this audience. Do NOT use generic "healthcare technology" — it is too broad and attracts the wrong people.
- **Lean into the Apple Notes / Transformation format for quality.** DCT_115 (AppleNotesTransformation) has an 82% qualification rate on DHW — the highest in the entire account. When you need quality over volume, this format is the answer.
- **Use the "Life Hack" format for volume when quality is already strong.** DCT_104 (Life Hack) delivers $2.80 CPL and 426 leads on DHW. It is a volume play, not a quality play. Pair it with a high-qualifying format to balance the portfolio.
- **Frame DHW as the filter between hype and signal.** The digital health space is full of AI/tech hype. DHW's value proposition is curation — separating genuinely impactful health tech developments from noise. Use language like: "context for smarter decisions," "what actually matters in health tech."
- **Iterate further on the "If you, BUT" conditional structure.** Lays and Jake discussed variations. Try: "If you make purchasing decisions for health IT systems BUT you're drowning in vendor pitches and conflicting analyst reports..." The conditional frame is a proven winner; the variable is what goes in the condition.

### DON'Ts
- **Do NOT use generic "healthcare technology" language on non-clinician DHW creative.** Jake was explicit: use "EHR systems, clinical software, or health IT." Generic language attracts nurses, clinicians, and patients — none of whom are the buyer persona DHW advertisers pay for.
- **Do NOT speak to nurses or doctors in non-clinician DHW ads.** The entire point of the non-clinician track is reaching the BUYERS of software, not the USERS. Nurses and doctors are users. Admins, IT leaders, and procurement are buyers. If your copy could appeal to an RN, it is wrong for non-clinician DHW.
- **Do NOT conflate DHW with CW or IW audience language.** DHW readers are executives and tech leaders, not practicing clinicians. "Improve patient outcomes" is a CW/IW phrase. DHW speaks to "operational efficiency," "vendor evaluation," "scaling digital health programs," "value-based care strategy."
- **Do NOT ignore the ebook lead magnet opportunity.** DCT_125 (EbookTeaser) is in the pipeline for DHW, using the Apple Notes format to tease the "Mastering Value-Based Care" ebook. This is a higher-intent conversion path. When the ebook LP goes live (target: first Monday of April), creative should be ready to launch immediately.
- **Do NOT use overly casual language.** DHW readers are C-suite and senior directors. They are more corporate than CW/IW audiences (who are clinicians). Copy should be polished-professional, not startup-casual.

---

## Universal Rules (All 3 Newsletters)

### NEVER Rules (Client-Mandated)
These are non-negotiable. Violating any of these will damage the client relationship.

1. **NEVER use overly casual or consumer-grade language.** This audience is MDs, PhDs, and executives. They react negatively to anything fluffy, oversimplified, or that feels like a BuzzFeed headline. If it could run as a consumer lifestyle ad, it is wrong for Insight Links.

2. **NEVER make clinical claims or promise specific patient outcomes.** Position the newsletter as information access and professional development — not clinical advice. "Stay current on the latest research" is fine. "Improve your patient outcomes" must be framed carefully as an aspiration, not a promise.

3. **NEVER use clickbait-style hooks.** Clinicians are, per Jake, "allergic to inbox clutter." The worst thing creative can resemble is a hospital-wide blast email or a spammy subject line. No "You won't BELIEVE what happened..." or "This ONE trick..."

4. **NEVER use AI-generated sounding copy.** Jake explicitly flagged wanting natural voice. The brand promise is "written in human voice" vs. legacy medical journals. If copy reads like it was generated by ChatGPT — smooth, generic, overly structured — it fails. Write like a knowledgeable colleague, not a language model.

5. **NEVER target or speak to patients, retired professionals, or students.** These are non-qualified audiences that have been an ongoing quality problem. Every word of copy should make clear this is for working healthcare professionals, not healthcare consumers.

6. **NEVER report or reference raw CPL without qualified CPL.** This is not a creative rule per se, but it reflects how the client thinks. If a creative is "winning" on raw CPL but losing on qualification rate, it is NOT winning. Jake: *"I'm all in on just like quality of lead... I would double our cost per lead and be okay with it."*

7. **NEVER let Imaging Wire creative go stale.** IW = 65% of revenue. Creative stagnation on IW without visible action is the pattern that killed Growletter's relationship. If IW performance slips, new concepts must be in-market within days, not weeks.

### Winning Formats (Ranked by Reliability)

**Tier 1 — Proven Winners:**
| Format | Best For | Example DCT | Key Metric | Notes |
|--------|----------|-------------|------------|-------|
| Text Callout Static | CW (volume + quality) | DCT_118 | $1.29 CPL, 4.09% CTR | Addresses "pressure to stay on top of everything" |
| "If you, BUT" Conditional | DHW (non-clinician targeting) | DCT_123 | $1.51 CPL, 3.26% CTR | Pre-qualifies with conditional framing |
| Transformation / Before-After | IW + DHW (quality) | DCT_114, DCT_115 | 75-82% qualification rate | Highest quality format in the account |

**Tier 2 — Reliable Performers:**
| Format | Best For | Example DCT | Key Metric | Notes |
|--------|----------|-------------|------------|-------|
| Confession-Style TOV | All 3 (volume) | DCT_103 | $1.90 CPL (CW) | FATIGUING — quality decay flagged by Jake Mar 19 |
| Life Hack TOV | DHW + CW (volume) | DCT_104 | $1.76-$2.80 CPL | Low qualification rates; use as volume anchor only |

**Tier 3 — In Pipeline (Untested):**
| Format | Target | Example DCT | Angle |
|--------|--------|-------------|-------|
| Private Briefing | IW | DCT_123_PrivateBriefing | Peer pressure + social proof |
| Cardiologist Filter | CW | DCT_124 | Professional filtering for qual rate |
| Ebook Teaser | DHW | DCT_125 | Lead magnet hook, Apple Notes format |
| Weekly Rounds | IW | DCT_126 | Confession + rounds metaphor |
| Noise Canceller | All 3 | DCT_127 | Subtraction: "Stop adding tabs. Start subtracting them." |

**Killed Formats (Do Not Resurrect):**
| Format | Why It Failed |
|--------|--------------|
| "Information Overload" (DCT_120) | Too vague. $5.52 CPL, 0.84% CTR. Clinicians need specificity, not abstract concepts. |
| "The Best [X] Newsletter" (DCT_109) | Superlative claims feel like a sales pitch. $12.15 CPL. Not peer-to-peer. |
| Generic "Why read X?" | Works adequately on DHW ($4.14 CPL) but tanks on IW ($7.36 CPL). Not reliable enough to invest in. |

### Voice & Tone Guide

**The Insight Links Voice = Knowledgeable Colleague, Not Marketing Brand**

Write as if you are a fellow healthcare professional sharing a useful resource — not a brand selling a subscription. The tone sits at the intersection of:
- **Professional** (not casual or consumer-grade)
- **Accessible** (not dense or academic like JACC or RSNA journals)
- **Time-respecting** (every word should signal "this saves you time")
- **Credible** (signal domain expertise, not generic convenience)

**Approved Phrases (Use These):**
- "Stay current"
- "Filter the noise"
- "Caught up fast"
- "Smarter while saving time"
- "Curated"
- "Context for smarter decisions"
- "What actually matters"
- "Easy and accessible"

**Voice-of-Customer Phrases (From Reddit Audience Research):**
- "Maze of information"
- "Higher yield"
- "So much junk"
- "I can never catch up"
- "Waste everyone's time"

Use these naturally in copy. They are the actual language healthcare professionals use to describe the problem these newsletters solve.

**Approved Hook Structures:**
1. **Confession:** "Confession: If you're a [role] who wants to [dream outcome], you need to [action]."
2. **Conditional "If you, BUT":** "If you [situation], BUT [qualifier], this is for you."
3. **Text Callout / Pain Point:** Address the feeling of needing to be on top of everything all the time.
4. **Transformation / Before-After:** Visual proof showing the old way (journals, tabs, PDFs) vs. the new way (one newsletter).
5. **Subtraction:** "Stop adding tabs. Start subtracting them." — flips the typical "add this to your routine" pitch.

**Copy Length & Format:**
- Text-heavy ads outperform visually complex creative for this audience
- Longer primary text is acceptable — these are professionals who read for a living
- Static > video for qualification rates (video drives volume but often at lower quality)
- TOV (text over video) is the best-performing video format when video is needed

**Failed Copy Signal (From Reddit Research):**
*"Summaries worry me unless I know they're done by people who understand the field."*

This means: do not position the newsletter as a generic news aggregator or AI summary tool. Always signal that the curation is done by domain experts who understand the clinical and business context.

### Quality vs. Volume Decision Framework

When writing creative for Insight Links, always ask: **"Does this ad attract the ICP, or does it attract anyone who works in healthcare?"**

- If the answer is "anyone in healthcare" — the creative is too broad. It will drive volume but tank qualification rates.
- If the answer is "specifically [cardiologists / radiologists / health IT buyers]" — the creative is correctly targeted.

Jake values quality over volume. He explicitly said he would **double his CPL** if it meant better leads. The copywriter's job is not to minimize CPL — it is to attract the right professionals at a reasonable cost.

**Quality Hierarchy:**
1. Transformation / Before-After — 75-82% qualification rate
2. Text Callout Static — high qualification + high volume (rare combination)
3. "If you, BUT" Conditional — pre-qualifies by design
4. Confession TOV — medium qualification, declining (creative fatigue)
5. Life Hack TOV — low qualification, high volume only

### Per-Newsletter Quick Reference

| Dimension | Cardiac Wire | Imaging Wire | Digital Health Wire |
|-----------|-------------|-------------|-------------------|
| **ICP** | Cardiologists, cardiac surgeons | Radiologists, imaging directors | Health tech execs, IT buyers |
| **Lead value to client** | Not disclosed | $150/lead to advertisers | $100/lead to advertisers |
| **Current qualified CPL** | $15.57 | $22.36 (ABOVE TARGET) | $13.44 |
| **CVR** | 47.39% | 34.59% (BELOW BENCHMARK) | 47.65% |
| **Top creative** | DCT_118 Text Callout | DCT_114 Replace Journals | DCT_123 "If you, BUT" |
| **Biggest risk** | 65+ age creep | Stagnation (65% of revenue) | Attracting clinicians instead of buyers |
| **Age targeting** | 25-59 (mandatory) | 25-59 | 25-59 |
| **Competitor landscape** | TCTMD, Medscape Cardiology | AuntMinnie, RSNA | Hospitalogy, Out of Pocket |
| **LP social proof** | 39,000+ | 32,000+ | 28,000+ |

---

*This guide should be updated whenever new DCTs prove out, new client feedback arrives, or creative performance shifts materially. Next review: after ebook lead magnet campaigns launch (target: first week of April 2026).*
