...
for (var i = 0; i < config.AGENCIES.length; i++) {
  ...
  var label = agency.name + " (" + agency.category_name + ")";
  ...
  console.log("Total: " + allContacts.length + " Bakeoff: " + bakeoffContacts.length);

  var sampleSize = Math.min(config.CONTACT_SAMPLE_SIZE, bakeoffContacts.length);
  var shuffled = bakeoffContacts.slice().sort(function() { return 0.5 - Math.random(); });
  var sample = shuffled.slice(0, sampleSize);
  ...
  for (var k = 0; k < sample.length; k++) {
    try {
      var detail = await $http.request({
        method: "GET",
        url: config.BASE_URL + "/Contacts/" + sample[k].MemberID_,
        headers: {
          username: config.USERNAME,
          password: config.PASSWORD,
          listname: config.LISTNAME
